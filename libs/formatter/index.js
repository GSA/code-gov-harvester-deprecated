...
const { transformStringToKey } = require('../utils');
const Logger = require('../logger');

const logger = new Logger({ name: 'formatter' });
...
class Formatter {
  ...
  _getUsageTypeExemptionText(repo) {
    const exemptionTexts = JsonFile.readFileSync(path.join(
      path.dirname(__dirname), './data/repo_upgrade_texts.json'));
  ...
  _formatRepo(repo){
    ...
    // repo name fields
    let repoId = transformStringToKey(
  ...
  formatRepo(repo, agencyMetadata, schemaVersion = '1.0.1') {
    return new Promise((resolve, reject) => {
      let formattedRepo;
      try {
        repo.agency = agencyMetadata;
